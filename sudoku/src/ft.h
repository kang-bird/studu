...
bool    box_check(char **arr, int x, int y, char num);  // проверка числа в квадрате 3х3
bool	num_check(char **arr, int x, int y, char num);  // проверка совпадения числа по всем фронтам
int		solve(char **arr, int *s_num);
char**	arr_cpy(char **arr_old);  // копирование массива
bool	double_check(char **arr);  // проверка совпадения всех чисел по всем фронтам
bool	preparing(int *x, int *y, char **new_arr);  // находим пустую ячейку
char	zero_check(char **new_arr, int x, int y);  // подсчёт кол-ва допустимых для вставки чисел
#endif
